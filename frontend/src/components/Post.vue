<template>
  <div class="card mb-4 w-75 mx-auto">
    <div class="card-header d-flex justify-content-between">
      <div>Post by <em class="text-secondary">{{User.pseudo}}</em> le <em class="text-secondary">{{post.createdAt.split(' ')[0]}}</em> à <em class="text-secondary">{{post.createdAt.split(' ')[1]}}</em></div>
      <div class="dropdown" v-if="user.isAdmin==true || user.pseudo == User.pseudo">
        <svg
